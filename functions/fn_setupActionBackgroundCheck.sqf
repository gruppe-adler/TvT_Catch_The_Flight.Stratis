...
GRAD_IdCard_doBackgroundCheck = {
    _target;
	["performing bg check on"] call Mission_fnc_showHint;
    ...
    if ([_background] call CBA_fnc_isHash) exitWith {
     ...
     [
        ...
        [_target, _varname],
        {
            ...
            TRACE_2("setting  background data; varname %1, val %2 ",_varname, _allegiances);
            ...
            _addPublishNode = {
                ...
                _action = [
                    "GRAD_IdCard_publish",
                    ("Publish information on " + (name _target)),
                    "",
                    {
                        _target = param [0, objNull];
                        _hash = _target getVariable _varname;
                        [_hash, "isPublic", true] call CBA_fnc_hashSet;
                        _target setVariable [_varname, _hash, true];
                    },
                    {true},
                    [_target]
                ] call ace_interact_menu_fnc_createAction;
                [player, 1, ["GRAD_IdCard_publishBackground"], _action] call ace_interact_menu_fnc_addActionToObject;
            };

            [_target] call _addPublishNode;
...
_action = ["GRAD_IdCard_publishBackground", "Publish collected background information", "", {}, {true}] call ace_interact_menu_fnc_createAction;
